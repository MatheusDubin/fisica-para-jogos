...
- **Multi-threading e paralelismo na Torre:** o job system do Jolt brilha com muitos islands independentes (Cenário 2). Uma pilha vertical forma um único island conectado, o que limita o paralelismo útil intra-island — isso é esperado e correto. O que é importante entender aqui é um detalhe arquitetural do Jolt (confirmado na GDC talk de Rouwé, 2022): **Jolt reconstrói islands do zero a cada frame**, de forma lock-free, sem manter estado de island entre frames. Unity (PhysX) e Unreal (Chaos) mantêm o estado de island entre frames (*warm starting* de island). Para a Torre isso significa: o padrão de convergência e sleep do Jolt pode diferir dos outros dois não por limitação de threading, mas pela ausência de warm starting de island state. Observe isso empiricamente — pode aparecer como tempo até sleep ligeiramente diferente ou padrão de jitter distinto.
